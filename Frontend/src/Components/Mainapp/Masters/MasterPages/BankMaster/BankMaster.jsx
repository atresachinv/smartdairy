...
//     } else {
//       toast.success("Failed to add new bank!");
//     }
//   };
...
//               <span className="label-text w15">{bank.ifsc}</span>
...
const BankMaster = () => {
  ...
  const handleAddBank = async (e) => {
    ...
    if (isEditMode) {
      ...
      const res = await dispatch(
        updateBankDetails({ id: editId, values })
      ).unwrap();
      if (res.status === 200) {
  ...
  //         </div>
  //       </form>
  //       <div className="bank-details-container w100 h70 d-flex-col bg">
  //         {bankList.map((bank, index) => (
  //           <div
  //             key={index}
  //             className="bank-details-headings w100 p10 d-flex a-center sa"
  ...
  //             <span>{bank.ifsc}</span>
  //             <button onClick={() => handleEditBank(bank)}>Edit</button>
          ...
          bankList.map((bank, index) => (
            ...
            >
              ...
              <span className="label-text w10 t-center">
                <span onClick={() => handleEditBank(bank)}></span>
                <span onClick={() => handleDeleteBank(bank.id)}></span>
              </span>
            </div>
          ))
        ) : (
