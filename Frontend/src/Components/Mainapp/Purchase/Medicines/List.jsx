...
const List = () => {
    ...
    const fetchPurchaseList = async () => {
      ...
      try {
        const response = await axiosInstance.get(
          `/purchase/all?itemgroupcode=2&cn=0&role=${userRole}`
        );
  ...
  const handleShowbutton = async () => {
    ...
    try {
      ...
      const { data } = await axiosInstance.get(
        `/purchase/all?ItemGroupCode=2&cn=0&role=${userRole}&${queryParams}`
      );
      // console.log(data);
      if (data?.success) {
  ...
  const downloadPdf = () => {
    ...
    const gepInfo = doc.getTextWidth("Medicines Report");
    doc.text("Medicines Report", (pageWidth - gepInfo) / 2, margin + 35);
    ...
    // Save the PDF
    doc.save(
      `Medicines_PurchaseReport_${formatDateToDDMMYYYY(
        date1
  ...
  return (
    <div className="purchase-product-bill-list-container w100 h1 d-flex-col sb p10">
      <span className="heading">औषधे खरेदी यादी :</span>
      ...
      <div className="purchase-prod-list-container w100 h65 d-flex-col bg">
        ...
        <div className="purchase-prod-detilas-table-div w100 h90 mh90 d-flex-col hidescrollbar">
