...
const List = () => {
    ...
    const fetchPurchaseList = async () => {
      ...
      try {
        const response = await axiosInstance.get(
          `/purchase/all?itemgroupcode=1&cn=0&role=${userRole}`
        );
  ...
  const handleShowbutton = async () => {
    ...
    try {
      ...
      const { data } = await axiosInstance.get(
        `/purchase/all?ItemGroupCode=1&cn=0&role=${userRole}&${queryParams}`
      );
      // console.log(data);
      if (data?.success) {
  ...
  const downloadPdf = () => {
    ...
    // Save the PDF
    doc.save(
      `Cattle_Feed_PurchaseReport_${formatDateToDDMMYYYY(
        date1
