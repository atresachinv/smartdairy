...
const CreateOthers = () => {
  ...
  const [rctno, setRctno] = useState(localStorage.getItem("receiptno4") || 1);
 ...
 `);
  ...
  const handleItemstoShow = () => {
    if (userRole !== "mobilecollector") {
      const items = productlist.filter((item) => item.ItemGroupCode === 4);
      setGroupItems(items);
  ...
  return (
    <div className="sale-products-list-container w100 h1 d-flex-col sb p10">
      <div className="page-title-center-select-container w100 h15 d-flex a-center">
        <span className="heading w20 px10">इतर वस्तु विक्री :</span>
        {userRole === "admin" ? (
          centerId > 0 ? (
            <></>
          ) : (
