...
const MedicinesSaleList = () => {
    ...
    const fetchSales = async () => {
      ...
      try {
        const { data } = await axiosInstance.get(
          `/sale/all?ItemGroupCode=2&cn=2&role=${userRole}`
  ...
  const handleShowbutton = async () => {
    ...
    try {
      ...
      const { data } = await axiosInstance.get(
        `/sale/all?ItemGroupCode=2&cn=2&role=${userRole}&${queryParams}`
      );
      if (data?.success) {
  ...
  const downloadPdf = () => {
    ...
    // Save the PDF
    doc.save(
      `Medicines_Expired_Report_${formatDateToDDMMYYYY(
        date1
  ...
  return (
    <div className="customer-list-container-div w100 h1 d-flex-col p10">
      ...
      <div className="sales-list-table w100 h80 d-flex-col hidescrollbar bg">
        <span className="heading p10"> {t("ps-cattle-medi-expired")}</span>
