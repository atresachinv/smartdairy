...
const CreateDealerGro = () => {
  ...
  const [rctno, setRctno] = useState(
    localStorage.getItem("dealretreceiptno3") || 1
  );
  ...
  const handleAddToCart = () => {
    if (selectitemcode > 0 && qty > 0 && rate > 0) {
      ...
      const newCartItem = {
        ...
        dealerCode: fcode,
        dealerName: cname,
        itemgroupcode: 3, // update in other form
        rate: rate,
        amount: qty * rate,
        cn: 1,
      };
  ...
  return (
    <div className="h1 w100 d-flex p10 sa a-center cust-return">
      <div className="custCol bg w45 h90">
        <span className="heading p10 "> {t("ps-addDealReturnGrocery")}</span>
...
export default CreateDealerGro;
