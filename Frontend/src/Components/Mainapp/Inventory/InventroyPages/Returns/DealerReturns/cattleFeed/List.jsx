...
const List = () => {
    ...
    const fetchPurchaseList = async () => {
      ...
      try {
        const response = await axiosInstance.get(
          `/purchase/all?cn=1&ItemGroupCode=1&role=${userRole}`
        );
  ...
  const handleShowbutton = async () => {
    ...
    try {
      ...
      const { data } = await axiosInstance.get(
        `/purchase/all?cn=1&ItemGroupCode=1&role=${userRole}&${queryParams}`
      );
      // console.log(data);
      if (data?.success) {
  ...
  const downloadPdf = () => {
    ...
    const gepInfo = doc.getTextWidth("CattleFeed Dealer Return Report");
    doc.text(
      "CattleFeed  Dealer Return Report",
      (pageWidth - gepInfo) / 2,
      margin + 35
    );
    ...
    // Save the PDF
    doc.save(
      `Cattle_Feed_Dealer_Return_Report_${formatDateToDDMMYYYY(
        date1
  ...
  return (
    <div className="dealer-return-list-container w100 h1 d-flex-col">
      <span className="heading">व्यापारी पशुखाद्य परत :</span>
      ...
      <div className="dealer-return-prod-list-container w100 h70 d-flex-col hidescrollbar bg">
        ...
        <div className="dealer-returns-prod-detilas-table-div w100 h1 mh100 d-flex-col hidescrollbar">
          ...
          {loading ? (
            ...
            <div className="w100 h1 d-flex center">{t("common:c-no-data-avai")}</div>
          )}
        </div>
      </div>
      {/* show invoice */}
