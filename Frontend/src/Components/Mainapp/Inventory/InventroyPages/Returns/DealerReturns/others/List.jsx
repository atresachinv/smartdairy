...
const OthList = () => {
    ...
    const fetchPurchaseList = async () => {
      ...
      try {
        const response = await axiosInstance.get(
          `/purchase/all?cn=1&ItemGroupCode=4&role=${userRole}`
        );
  ...
  const handleShowbutton = async () => {
    ...
    try {
      ...
      const { data } = await axiosInstance.get(
        `/purchase/all?cn=1&ItemGroupCode=4&role=${userRole}&${queryParams}`
      );
      // console.log(data);
      if (data?.success) {
  ...
  return (
    <div className="dealer-return-list-container w100 h1 d-flex-col">
      <span className="heading">व्यापारी इतर वस्तू परत :</span>
