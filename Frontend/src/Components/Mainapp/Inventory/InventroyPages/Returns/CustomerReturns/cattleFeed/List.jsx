...
const List = () => {
    ...
    const fetchSales = async () => {
      ...
      try {
        const { data } = await axiosInstance.get(
          `/sale/all?cn=1&ItemGroupCode=1&role=${userRole}`
  ...
  const handleShowbutton = async () => {
    ...
    try {
      ...
      const { data } = await axiosInstance.get(
        `/sale/all?ItemGroupCode=1&cn=1&role=${userRole}&${queryParams}`
      );
      if (data?.success) {
  ...
  const downloadPdf = () => {
    ...
    const gepInfo = doc.getTextWidth("CattleFeed Customer Return Report");
    doc.text(
      "CattleFeed Customer Return Report",
      (pageWidth - gepInfo) / 2,
      margin + 35
    );
    ...
    // Save the PDF
    doc.save(
      `Cattle_Feed_Cust_Return_Report_${formatDateToDDMMYYYY(
        date1
  ...
  return (
    <div className="customer-return-prod-list-container w100 h1 d-flex-col">
      <span className="heading">उत्पादक पशुखाद्य परत :</span>
