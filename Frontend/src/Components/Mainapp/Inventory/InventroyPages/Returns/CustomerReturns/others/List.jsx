...
const OthList = () => {
    ...
    const fetchSales = async () => {
      ...
      try {
        const { data } = await axiosInstance.get(
          `/sale/all?cn=1&ItemGroupCode=4&role=${userRole}`
  ...
  const handleShowbutton = async () => {
    ...
    try {
      ...
      const { data } = await axiosInstance.get(
        `/sale/all?ItemGroupCode=4&cn=1&role=${userRole}&${queryParams}`
      );
      if (data?.success) {
  ...
  const downloadPdf = () => {
    ...
    const gepInfo = doc.getTextWidth("Other Customer Return Report");
    doc.text(
      "Other Customer Return Report",
      (pageWidth - gepInfo) / 2,
      margin + 35
    );
    ...
    // Save the PDF
    doc.save(
      `Other_Cust_Return_Report_${formatDateToDDMMYYYY(
        date1
