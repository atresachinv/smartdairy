...
const MedCreate = () => {
  ...
  const [rctno, setRctno] = useState(
    localStorage.getItem("custretreceiptno3") || 1
  );
  ...
  const handleAddToCart = () => {
    if (selectitemcode > 0 && qty > 0 && rate > 0) {
      ...
      const newCartItem = {
        ...
        Qty: qty,
        CustCode: fcode,
        ItemGroupCode: 2, //update this to the actual item group code
        Rate: rate,
        Amount: qty * rate,
        cn: 1,
      };
    ...
    if (cartItem.length > 0) {
      try {
        ...
        if (res?.data?.success) {
          ...
          localStorage.setItem("custretreceiptno3", parseInt(rctno) + 1);
        }
      } catch (error) {
  ...
  return (
    <div className="h1 w100 d-flex p10 sa a-center cust-return">
      <div className="custCol bg w45 h80">
        <span className="heading p10"> {t("ps-addCustReturnMedi")}</span>
