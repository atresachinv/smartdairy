...
const OthCreate = () => {
  ...
  const [rctno, setRctno] = useState(
    localStorage.getItem("custretreceiptno4") || 1
  );
    ...
    if (cartItem.length > 0) {
      try {
        ...
        if (res?.data?.success) {
          ...
          localStorage.setItem("custretreceiptno4", parseInt(rctno) + 1);
        }
      } catch (error) {
  ...
  return (
    <div className="h1 w100 d-flex p10 sa a-center cust-return">
      <div className="custCol bg w45 h80">
        <span className="heading p10"> {t("ps-addCustReturnOther")}</span>
