...
const GroList = () => {
    ...
    const fetchSales = async () => {
      ...
      try {
        const { data } = await axiosInstance.get(
          `/sale/all?cn=1&ItemGroupCode=3&role=${userRole}`
  ...
  const handleShowbutton = async () => {
    ...
    try {
      ...
      const { data } = await axiosInstance.get(
        `/sale/all?&ItemGroupCode=3&cn=1&role=${userRole}&${queryParams}`
      );
      if (data?.success) {
  ...
  return (
    <div className="customer-return-prod-list-container w100 h1 d-flex-col">
      <span className="heading">उत्पादक किराणा परत :</span>
